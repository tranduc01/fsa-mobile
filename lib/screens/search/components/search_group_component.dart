...
class SearchGroupComponent extends StatelessWidget {
  ...
  const SearchGroupComponent(
      {required this.showRecent, this.callback, required this.groupList});
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        if (groupList.isNotEmpty && showRecent)
          ...
          AnimatedListView(
            ...
            itemBuilder: (context, index) {
              ...
              return SearchCardComponent(
                ...
                image: group.avatarUrls != null
                    ? group.avatarUrls!.full.validate()
                    : AppImages.defaultAvatarUrl,
                subTitle:
                    parseHtmlString(group.description!.rendered.validate()),
                callback: () {
                  callback?.call();
                },
              ).paddingSymmetric(vertical: 8).onTap(() async {
                if (!appStore.recentGroupsSearchList.any((element) =>
                    element.id.validate() == groupList[index].id)) {

      ...
      child: PopupMenuButton(
        ...
        onSelected: (val) async {
          if (val == 1) {
            ...
            AddPostScreen(
              showMediaOptions:
                  widget.post.type == PostActivityType.activityShare
                      ? false
                      : true,
              post: widget.post,
              ...
              component:
                  widget.groupId != null ? Component.groups : Component.members,
              callback: () {
        ...
        itemBuilder: (context) => <PopupMenuEntry>[
          ...
          if (widget.post.userId.validate().toString() !=
                  appStore.loginUserId &&
              widget.showHidePostOption!)
            PopupMenuItem(
              value: 4,
              child: TextIcon(
                  text: language.hidePost,
                  textStyle: secondaryTextStyle(),
                  prefix: Icon(Icons.visibility_off_outlined,
                      color: appStore.isDarkMode ? bodyDark : bodyWhite,
                      size: 16)),
              textStyle: primaryTextStyle(),
            ),
          PopupMenuItem(
            value: 5,
            child: TextIcon(
                text: widget.post.isFavorites == 1
                    ? language.unfavorite
                    : language.favourite,
                textStyle: secondaryTextStyle(),
                prefix: Image.asset(ic_star,
                    color: appStore.isDarkMode ? bodyDark : bodyWhite,
                    width: 16,
                    height: 16)),
            textStyle: primaryTextStyle(),
          ...
          PopupMenuItem(
            value: 6,
            child: TextIcon(
                text: widget.post.isPinned == 1
                    ? language.unpin
                    : language.pinToTop,
                textStyle: secondaryTextStyle(),
                prefix: Image.asset(ic_push_pin,
                    color: appStore.isDarkMode ? bodyDark : bodyWhite,
                    width: 16,
                    height: 16)),
            textStyle: primaryTextStyle(),
          ...
          PopupMenuItem(
            value: 7,
            child: TextIcon(
                text: language.shareOnActivity,
                textStyle: secondaryTextStyle(),
                prefix: Image.asset(ic_share,
                    color: appStore.isDarkMode ? bodyDark : bodyWhite,
                    width: 16,
                    height: 16)),
            textStyle: primaryTextStyle(),

...
class _PostComponentState extends State<PostComponent> {
  ...
  Future<void> postReaction(
      ...
      int? reactionID,
      bool fromQuickView = false}) async {
    ifNotTester(() async {
      if (addReaction) {
        ...
        await deletePostReaction(
                id: widget.post.activityId.validate(), isComments: false)
            .then((value) {
          if (fromQuickView) widget.callback!.call();
        }).catchError((e) {
          log('Error: ${e.toString()}');
      ...
      onAccept: (c) async {
        ifNotTester(() async {
          ...
          await removeExistingFriendConnection(
                  friendId: widget.post.userId.toString(), passRequest: true)
              .then((value) {
            appStore.setLoading(false);
            widget.callback?.call();
            setState(() {});
  ...
  Future<void> onFavorites() async {
    ifNotTester(() {
      ...
      favoriteActivity(postId: widget.post.activityId.validate().toInt());
    });
  }
  ...
  Future<void> onPinned() async {
    ifNotTester(() {
      pinActivity(
          postId: widget.post.activityId.validate(),
          pinActivity: widget.post.isPinned == 1 ? 0 : 1);
      ...
      return GestureDetector(
        onTap: () {
          if (widget.post.type.validate() == PostActivityType.newBlogPost) {
            ...
            SinglePostScreen(postId: widget.post.activityId.validate())
                .launch(context)
                .then((value) {
                ...
                builder: (context) {
                  return QuickViewPostWidget(
                    ...
                    onReactionRemoved: () {
                      isReacted = false;
                      postReaction(
                          reactionID: widget.post.curUserReaction!.id.toInt(),
                          addReaction: false);
                    },
                    pageIndex: index,

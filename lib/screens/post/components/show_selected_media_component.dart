...
class ShowSelectedMediaComponent extends StatefulWidget {
  ...
  const ShowSelectedMediaComponent(
      {required this.mediaType,
      required this.mediaList,
      required this.videoController});

  @override
  State<ShowSelectedMediaComponent> createState() =>
      _ShowSelectedMediaComponentState();
}

class _ShowSelectedMediaComponentState
    extends State<ShowSelectedMediaComponent> {
  @override
  void setState(fn) {
        ...
        itemBuilder: (context, index) {
          ...
          return Padding(
            ...
            child: Stack(
              children: [
                cachedImage(
                        media.isLink ? media.link : media.file!.path.validate(),
                        height: 80,
                        width: 80,
                        fit: BoxFit.cover)
              ...
              children: [
                widget.videoController[index].value.isInitialized
                    ? Container(
                        width: 80,
                        height: 80,
                        child: VideoPlayer(widget.videoController[index])
                            .cornerRadiusWithClipRRect(commonRadius),
                      )
                    : Container(
